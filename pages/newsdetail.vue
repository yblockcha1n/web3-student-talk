  ...
  methods: {
    ...
    async fetchNewsDetail(id) {
      ...
      try {
        ...
        // ニュース記事の内容に基づいてOGP設定を動的に生成
        this.setupOgp(result);
    ...
    setupOgp(newsItem) {
      ...
      // 記事のタイトルを設定
      document.title = `${newsItem.title} - Web3学生トーク`;
      
      // HTMLからテキストのみを抽出して説明文に使用
      const description = this.getContentText(newsItem.content, 100);
      ...
      // 記事固有のメタデータを追加で設定（公開日など）
      const metaTags = [
        { property: 'article:published_time', content: newsItem.publishedAt },
        { property: 'article:modified_time', content: newsItem.updatedAt }
      ];
      
      // カテゴリ情報があれば追加
      if (newsItem.category) {
        metaTags.push({ property: 'article:section', content: newsItem.category.name });
      }
